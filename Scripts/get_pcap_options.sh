...
#
# This is a script to pull all pcaps off a firewall, does skip files that already in the directory
#
...
# which PCAPs to get
function select_pcap_category {
  echo -e "${CYAN}Select PCAP category:${NC}"
  echo "1) application-pcap"
  echo "2) threat-pcap"
  echo "3) dlp-threat"
  echo "4) filter-pcap"
  ...
  case "$choice" in
    1) CATEGORY="application-pcap" ;;
    2) CATEGORY="threat-pcap" ;;
    3) CATEGORY="dlp-threat" ;;
    4) CATEGORY="filter-pcap" ;;
    *)
       log_warn "Invalid choice. Defaulting to application-pcap."
       CATEGORY="application-pcap"
       ;;
  esac

  log_info "Using category: $CATEGORY"
}
...
function get_pcaps_from_dir {
  ...
  response=$(curl -s -k -X POST "https://$FW_IP/api/?type=export&category=$CATEGORY&from=/$stripped_dir&key=$API_KEY")
  if [[ -z "$response" ]]; then
...
function download_pcaps {
  ...
  while IFS= read -r pcap_path; do
    ...
    curl -s -k -X POST "https://$FW_IP/api/?type=export&category=$CATEGORY&from=$pcap_path&key=$API_KEY" -o "$dest_path"
    if [[ $? -eq 0 ]]; then
